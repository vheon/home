...
vim.api.nvim_create_autocmd({'BufWinEnter', 'FileType'}, {
    ...
    callback = function()
        ...
        if vim.tbl_contains(ignore_buftype, vim.bo[buf].buftype) then
            return
        end

        if vim.tbl_contains(ignore_filetype, vim.bo[buf].filetype) then
        ...
        local last_line = unpack(vim.api.nvim_buf_get_mark(buf, '"'))
